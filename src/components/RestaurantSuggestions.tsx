...
export default function RestaurantSuggestions() {
    ...
    const dynamicRestaurants = [
        ...
        },
        {
            ...
            priceAvg: 15,
            icon: <UtensilsCrossed className="w-5 h-5 text-sky-500" />,
            color: "bg-sky-50"
        },
        {
            id: "r5",
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col mt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                {dynamicRestaurants.map((restaurant) => {
                    ...
                    return (
                        <div key={restaurant.id} className="border border-slate-100 rounded-xl p-4 flex flex-col group sm:hover:shadow-md transition-all sm:hover:border-rose-200">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-3 ${restaurant.color}`}>
                                {restaurant.icon}
                            </div>

                            <h3 className="font-semibold text-slate-900 leading-snug">{restaurant.name}</h3>
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mt-1">{restaurant.type}</span>
                            ...
                            <div className="flex flex-col gap-3 pt-4 border-t border-slate-100 mt-auto">
                                <a
                                    ...
                                    className="text-[10px] font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg text-center transition-colors flex items-center justify-center gap-1.5"
                                >
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex flex-col">
                                        <span className="font-bold text-lg text-slate-800 tracking-tight">
                                            {restaurant.priceAvg} <span className="text-xs font-normal text-slate-500">{config.currency}</span>
                                        </span>
                                    </div>
